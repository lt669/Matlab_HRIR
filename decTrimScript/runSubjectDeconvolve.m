...
subjectSweepsPath = sprintf('EAD_HRIR/Audio/SubjectSweeps_Raw/%s/',subjectName); % Used to load files
...
mkdir(sprintf('EAD_HRIR/Audio/HRIR_Raw/%s',subjectName));
mkdir(sprintf('EAD_HRIR/Audio/HRIR_Trim/%s',subjectName));
...
for n = 1:length(subjectDir)
    ...
    % Write Raw HRIR
    outputRaw(:,:) = decNorm(n,:,:);
    name = strcat('EAD_HRIR/Audio/HRIR_Raw/',subjectName,'/',char(fileName(n)),'_RawLong.wav');
    ...
    name = strcat('EAD_HRIR/Audio/HRIR_Trim/',subjectName,'/',char(fileName(n)),'_Raw.wav');
